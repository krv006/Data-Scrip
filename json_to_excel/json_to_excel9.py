...
def flatten_json(data: Dict) -> Dict:
    ...
    for item in data.get('label_values', []):
        ...
        if 'value' in item:
            ...
            # Takrorlanuvchi labellarni farqlash uchun
            key = label if label not in flat_dict else f"{label}_{len([k for k in flat_dict.keys() if k.startswith(label)]) + 1}"
            flat_dict[key] = (
                datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                if ts > 0 else 'Not set'  # 0 qiymati uchun maxsus ishlov berish
            )
        elif 'vec' in item:
...
def process_json_file(input_file: str, output_file: str) -> None:
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            df.to_excel(writer, sheet_name='Caption Data', index=False)

            # XlsxWriter ob'ektlarini olish
            worksheet = writer.sheets['Caption Data']
            ...
            for i, col in enumerate(df.columns):
                ...
                width = 50  # Standart kenglik
                worksheet.set_column(i, i, width, text_format if col in ['Text', 'Original text'] else None)
...
if __name__ == "__main__":
    input_file = 'video_captions.json'
    output_file = 'video_captions.xlsx'
